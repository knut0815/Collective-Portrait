...
void main() {
    ...
    vec4 paintContribution = mix(sample1, sample2, 0.5);
    
    // Edges are black (0.0) everything else in this sampler is white (1.0)
    vec4 edge = 1.0 - texture(tex2, vs_texcoord);
    vec4 edgeContribution = mix(vec4(0.3, 0.5, 0.8, 1.0), vec4(1.0), edge.r) * 0.1; // Changing this multiplier and color does weird shit
    
    // Sample the paper texture
    vec4 background = texture(tex3, vs_texcoord);
    
    out_color = vec4((paintContribution * background + edgeContribution).rgb, 1.0); // Changing the addition to subtraction does more weird shit
}